...
const searchMentor = async (offset, limit, key) => {
    let statement = `
    ...
    if(key !== undefined && key !== "undefined"){
        statement += " AND UPPER(CONCAT(first_name, ' ', last_name)) LIKE CONCAT('%', UPPER(?), '%')";
        params.push(key);
    }
...
const searchCourse = async (userId, offset, limit, key, tagNameContains, orderByRating, orderByEnrollmentCount, orderByCurrentPrice) => {
    let statement = `
    SELECT 
    ...
    if(key !== undefined && key !== "undefined"){
        statement += " AND UPPER(course.name) LIKE CONCAT('%', UPPER(?), '%')";
        params.push(key);
    }
